...
class Recoder(object):
  ...
  def _train(self, train_dataloader, val_dataloader,
    ...
    for epoch in range(current_epoch, num_epochs):
      ...
      out_of_critical_regime, status_changed = self.check_critical_regime_hardcode(epoch, num_epochs, batch_size, self.original_batch_size)
      ...
      if self._enable_gavel_iterator:
        ...
        if status_changed:
          if out_of_critical_regime and batch_size != 8192:
            train_dataloader.update_resource_requirement(big_bs=True, small_bs=False)
          # # this case won't happen due to the nature of Recommendation workloads
          # elif not out_of_critical_regime and batch_size != 512:
          #   train_dataloader.update_resource_requirement(big_bs=False, small_bs=True)
  ...
  def check_critical_regime_hardcode(self, epoch, total_num_epochs, current_batch_size, original_batch_size):
    # TODO: Run physical tests to make sure the correctness
    if original_batch_size in [512, 1024]:
        regime = 30
    elif original_batch_size in [2048]:
        regime = 40
