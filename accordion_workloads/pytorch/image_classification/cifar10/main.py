...
# Training
def train(epoch, full_rank_accum, cumulative_steps=None, cumulative_time=None):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        gather_grad_array(net, full_rank_accum)
    ...
    return (cumulative_steps, cumulative_time, done, finished_epoch, num_completed_iters, full_rank_accum)
...
def check_critical_regime_hardcode(epoch, total_num_epochs, current_batch_size, original_batch_size):
    ...
    if original_batch_size in [16, 32, 64, 128]:
        regime = 10
    elif original_batch_size in [256]:
        regime = 20
    critical_regime = list(range(regime)) + list(range(150, 160)) + list(range(250, 260))
...
def linear_learning_rate(optimizer, big_bs, small_bs):
    assert(big_bs != small_bs)
    for group in optimizer.param_groups:
            group['lr'] = group['lr'] * (2 if big_bs else 0.5)
...
for epoch in range(start_epoch, args.num_epochs):
    ...
    # full_rank_accum = [torch.zeros_like(copy_l) for copy_l in net.parameters()]
    (cumulative_steps, cumulative_time, done, finished_epoch, num_completed_iters, full_rank_accum) =\
            train(epoch, full_rank_accum, cumulative_steps, cumulative_time)
    ...
    out_of_critical_regime, status_changed = check_critical_regime_hardcode(epoch, args.num_epochs, args.batch_size, original_batch_size)

    if args.enable_gavel_iterator:
        # # FIXME: debug
        # if epoch in [0, 1, 2, 3]: 
        # # if epoch in [0, 1, 2, 3] and args.rank == 0: 
        # # if epoch in [0, 1]: 
