...
class Dispatcher:
    ...
    def _configure_numa(self):
        ...
        for i in range(num_gpus):
            ...
            self._logger.info(
                "GPU {gpu} assigned CPUs {cpus}".format(
    ...
    def _get_iterator_log(self, job_id, worker_id, round_id):
        ...
        if not os.path.exists(log_file):
            self._logger.error(
                "Could not read GavelIterator log file for "
                "job {job_id} (worker {worker_id}, "
                "round {round_id})!".format(
                    job_id=job_id, worker_id=worker_id, round_id=round_id
                )
            )
            return ""
    ...
    def launch_job(
        self, job, prefix, command, worker_id, round_id, gpu_id, mode
    ):
        output = ""
        if mode == "static":
        ...
        self._logger.debug(
            f"[Job {job.job_id}] Round {round_id}, cwd on worker {worker_id} is {cwd}"
        )
        ...
        try:
            ...
            error_message = (
                "Job {job_id} (worker {worker_id}, "
                "round {round_id}) failed!"
            ).format(job_id=job.job_id, worker_id=worker_id, round_id=round_id)
            ...
            self._logger.error(
                "Dispatcher failed to launch job {job_id} "
        ...
        try:
            ...
            traceback.print_exc()
            self._logger.error(
                "Could not get steps and execution time for "
                "job {job_id} (worker {worker_id},  "
                "round {round_id})!".format(
                    job_id=job.job_id, worker_id=worker_id, round_id=round_id
                )
    ...
    def _dispatch_jobs_helper(self, jobs, worker_id, round_id):
        ...
        self._logger.debug(
            "Constructing commands for " "worker {0}...".format(worker_id)
        )
        ...
        for job in jobs:
            try:
                prefix, command = self._construct_command(
                    job, gpu_id, worker_id
                )
        ...
        if success:
            ...
            for job, (prefix, command) in zip(jobs, commands):
                self._logger.info(
                    ...
                    "prefix: {prefix}, "
                    "mode: {mode}, "
                    "MPS thread percentage: {mps_thread_percentage}%, "
                    'command: "{command}"'.format(
                        ...
                        mode=job.mode,
                        mps_thread_percentage=job.mps_thread_percentage,
                        command=command,
                    )
                )
                results.append(
    ...
    def dispatch_jobs(self, jobs, worker_id, round_id):
        self._thread_pool.apply_async(
            self._dispatch_jobs_helper, (jobs, worker_id, round_id,)
        )
