        ...
        try:
            ...
            self._logger.info(
                ...
                "with id(s) {worker_ids}".format(
                    worker_type=request.worker_type, worker_ids=str(worker_ids)
                )
            )
...
class SchedulerIteratorRpcServer(i2s_pb2_grpc.IteratorToSchedulerServicer):
    ...
    def InitJob(self, request, context):
        ...
        self._logger.info(
            f"max_steps, max_duration, extra_time is {max_steps}, {max_duration}, {extra_time}"
        )
        ...
        return i2s_pb2.UpdateLeaseResponse(
            max_steps=max_steps,
            max_duration=max_duration,
            extra_time=extra_time,
        )
    ...
    def UpdateLease(self, request, context):
        ...
        try:
            ...
            self._logger.info(
                ...
                "max_duration={max_duration:.2f}, "
                "job run time so far={run_time_so_far}, "
    ...
    def UpdateResourceRequirement(self, request, context):
        job_id = JobIdPair(request.job_id, None)
        self._logger.info(
            "Received resource requirement update request: "
            "job_id={job_id}, worker_id={worker_id}, asking to "
            "go into {big_or_small} batch size".format(
                job_id=job_id,
                worker_id=request.worker_id,
                big_or_small=("big" if request.big_bs else "small"),
            )
        )
        update_resource_requirement_callback = self._callbacks[
            "UpdateResourceRequirement"
        ]
        # TODO: think about what parameters the callback function in scheduler.py will need
        update_resource_requirement_callback(
            job_id, request.worker_id, request.big_bs, request.small_bs
        )
        return common_pb2.Empty()
