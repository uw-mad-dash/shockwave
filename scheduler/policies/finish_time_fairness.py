...
class FinishTimeFairnessPolicy(Policy):
    def __init__(self, solver):
        ...
        self._finish_time_fairness_perf_policy = FinishTimeFairnessPolicyWithPerf(
            solver
        )
...
class FinishTimeFairnessPolicyWithPerf(Policy):
    ...
    def get_allocation(
        ...
        for i in range(len(job_ids)):
            ...
            expected_time_isolated = self._cumulative_isolated_time[
                job_ids[i]
            ] + (num_steps_remaining[job_ids[i]] / isolated_throughputs[i])
            expected_time_allocation = times_since_start[job_ids[i]] + (
                num_steps_remaining[job_ids[i]]
                * cp.inv_pos(allocation_throughput)
            )
            expected_time_fraction = (
                expected_time_allocation / expected_time_isolated
            )
        ...
        try:
            result = cvxprob.solve(solver=self._solver, verbose=True)
            ...
            return super().unflatten(
                x.value.clip(min=0.0).clip(max=1.0), index
            )
