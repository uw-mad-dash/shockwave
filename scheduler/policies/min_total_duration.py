...
class MinTotalDurationPolicy(Policy):
    def __init__(self, solver):
        ...
        self._min_total_duration_perf_policy = MinTotalDurationPolicyWithPerf(
            solver
        )
    ...
    def get_allocation(
        ...
        for job_id in unflattened_throughputs:
            ...
            for worker_type in unflattened_throughputs[job_id]:
                new_unflattened_throughputs[job_id][
                    worker_type
                ] = unflattened_throughputs[job_id][
                    "v100"
                ]  # why?
...
class MinTotalDurationPolicyWithPerf(Policy):
    ...
    def get_allocation_helper(self, throughputs, scale_factors_array, T):
        # print(f"#####Throughputs received by optimizer is {throughputs}")
        # print(f"#####Throughputs shape is {throughputs.shape}")
        x = cp.Variable(
            throughputs.shape
        )  # size of x is (num_jobs, accerlator_types)?
        ...
        constraints = self.get_base_constraints(
            x, scale_factors_array
        )  # make sure the allocation fits in the scheduler
        constraints.append(
        ...
        # print(f"########status: {cvxprob.status}")
        # if cvxprob.status == "optimal" and x is not None:
        # # if x is not None and x.value is not None:
        #     print(f"x: {x.value.clip(min=0.0).clip(max=1.0)}")
    ...
    def get_allocation(
        ...
        (job_ids, _) = index
        self._num_steps_remaining = np.array(
            [num_steps_remaining[job_id] for job_id in job_ids]
        )
        if throughputs is None:
            return None
        ...
        while last_feasible_x is None:
            ...
            while (
                1.05 * min_T
            ) < max_T:  # TODO: Can tweak the 1.05 in this loop.
                T = (min_T + max_T) / 2.0
                # print(f"#####Searching for T {T}")
                status, x = self.get_allocation_helper(
                    throughputs, scale_factors_array, T
                )
                if status == "optimal":
                    last_feasible_x = x
                    max_T = T
...
class MinTotalDurationPolicyWithPacking(PolicyWithPacking):
    ...
    def get_allocation_helper(
        ...
        constraints = self.get_base_constraints(
            x, single_job_ids, scale_factors_array, relevant_combinations
        )

        # See if passed in T is feasible.
        for i, (throughputs, num_steps_remaining) in enumerate(
            zip(all_throughputs, self._num_steps_remaining)
        ):
    ...
    def get_allocation(
        ...
        all_throughputs, index = super().flatten(
            unflattened_throughputs, cluster_spec
        )
        ...
        self._num_steps_remaining = [
            num_steps_remaining[single_job_id]
