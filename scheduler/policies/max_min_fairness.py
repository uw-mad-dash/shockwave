...
class MaxMinFairnessPolicyWithPacking(PolicyWithPacking):
    ...
    def get_allocation_using_job_type_throughputs(
        ...
        flattened_scale_factors = np.reshape(
            [scale_factors[job_id] for job_id in job_ids], (n, 1)
        )
        scale_factors_array = np.tile(
            flattened_scale_factors, (1, num_vars_per_job * m)
        )

        # Set up flattened job type throughputs.
        flattened_throughputs = np.zeros(
            shape=(a, (1 + a) * m), dtype=np.float32
        )
                ...
                for j, other_job_type_key in enumerate([None] + job_type_keys):
                    if j > 0 and other_job_type_key[1] != job_type_key[1]:
                        ...
                        flattened_throughputs[
                            i, k * (1 + a) + j
                        ] = unflattened_throughputs[job_type_key][worker_type][
                            other_job_type_key
                        ]
        ...
        for i in range(m):
            relevant_vars = x[
                :, i * num_vars_per_job : (i + 1) * num_vars_per_job
            ]
            relevant_scale_factors = scale_factors_array[
                :, i * num_vars_per_job : (i + 1) * num_vars_per_job
            ]
        ...
        for i, job_id in enumerate(job_ids):
            ...
            all_coefficients[i] = np.multiply(
                flattened_throughputs[job_type_idx], scale_factors_array[i]
            ) / (
                unflattened_priority_weights[job_id] * proportional_throughput
            )
        objective = cp.Maximize(
            cp.min(cp.sum(cp.multiply(all_coefficients, x), axis=1))
        )
        ...
        return self.convert_job_type_allocation(
