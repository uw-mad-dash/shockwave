        ...
        while len(queue) > 0:
            ...
            for scheduled_job_id in self._allocation:
                ...
                merged_job_id = job_id_pair.JobIdPair(
                    scheduled_job_id[0], job_id_to_schedule[0]
                )
                ...
                for i, single_job_id in enumerate(merged_job_id.singletons()):
                    if packed_throughput[i] <= 0.0:
                        continue
                    isolated_throughput = throughputs[single_job_id][
                        worker_type
                    ]
            ...
            if job_id_to_pack_with is None:
                ...
                self._output = None
                merged_job_id = job_id_pair.JobIdPair(
                    job_id_to_pack_with[0], job_id_to_schedule[0]
                )
                ...
                if len(queue) > 0:
                    ...
                    if (
                        ...
                        if throughputs[job_id_to_schedule][worker_type] > 0.0:
                            ...
                            available_workers[worker_type] -= scale_factors[
                                job_id_to_schedule
                            ]
        ...
        while len(queue) > 0 and len(available_worker_types) > 0:
            ...
            for i, worker_type in enumerate(available_worker_types):
                if available_workers[worker_type] >= scale_factor:
                    available_worker_types_with_scale_factor.append(
                        worker_type
                    )
            ...
            if throughputs[job_id_to_schedule][worker_type] > 0.0:
                ...
                if available_workers[worker_type] == 0:
                    worker_type_idx = original_available_worker_types_mapping[
                        worker_type_idx
                    ]
...
class FIFOPolicyWithPacking(PolicyWithPacking):
    def __init__(self, packing_threshold=1.5):
        ...
        self._policy = FIFOPolicy(
            mode="packing", packing_threshold=packing_threshold
        )
