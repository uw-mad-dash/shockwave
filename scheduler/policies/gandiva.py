...
class GandivaPolicy(PolicyWithPacking):
    ...
    def _get_allocation(
        self, job_combinations_to_schedule, index, scale_factors, cluster_spec
    ):
    ...
    def _get_normalized_throughput(
        self, job_combination, throughputs, worker_types
    ):
    ...
    def get_allocation(
        ...
        for job_id in assigned_combination_keys:
            ...
            if (
                self._get_normalized_throughput(
                    job_combination, unflattened_throughputs, worker_types
                )
                < 1.0
            ):
                to_delete.extend([job_id, other_job_id])
        ...
        if num_workers_requested <= num_workers_available:
            ...
            x = self._get_allocation(
                single_job_ids, index, scale_factors, cluster_spec
            )
        else:
            # Deploy packing.
            ...
            while len(to_be_assigned_combinations) > 1 and i < n:
                ...
                self._assigned_combinations[job1_id] = (
                    job_combination,
                    job2_id,
                )
                self._assigned_combinations[job2_id] = (
                    job_combination,
                    job1_id,
                )
