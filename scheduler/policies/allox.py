    ...
    def get_allocation(
        ...
        for job_id in unflattened_throughputs:
            if job_id not in self._prev_allocation:
                ...
                for worker_type in worker_types:
                    total_allocation += self._prev_allocation[job_id][
                        worker_type
                    ]
            ...
            # # enable preemptive round schedule
            # unallocated_job_ids.append(job_id)
        ...
        for worker_type in worker_types:
            ...
            for already_allocated_job_id in already_allocated_job_ids:
                if (
                    self._prev_allocation[already_allocated_job_id][
                        worker_type
                    ]
                    == 1.0
                ):
        ...
        ######################################################################
        # job_share = {}
        # num_gpus = cluster_spec["v100"]
        # num_completed_rounds = len(per_round_schedule)

        # for job_id in unallocated_job_ids:
        #     job_id = job_id.integer_job_id()
        #     num_scheduled_rounds = 0
        #     for round_schedule in per_round_schedule:
        #         if job_id in round_schedule.keys(): num_scheduled_rounds += 1
        #     if num_completed_rounds <= 0:
        #         share = 0.0
        #     else:
        #         share = num_scheduled_rounds * scale_factors[job_id]

        #     job_share[job_id] = share

        # # sort job ids according to their share from least to most, as defined in the allox paper
        # unallocated_job_ids.sort(key=lambda x: job_share[x])
        ######################################################################
        # Sort job IDs according to arrival time.
        unallocated_job_ids.sort(key=lambda x: -times_since_start[x])
        unallocated_job_ids = unallocated_job_ids[
            : max(int(self._alpha * m), n)
        ]
        m = len(unallocated_job_ids)
        ...
        for i in range(m):
            for j in range(n):
                ...
                q_base[i, j] = (
                    num_steps_remaining[unallocated_job_ids[i]] / throughput
                )
        ...
        for (row_index, col_index) in zip(row_indices, col_indices):
            ...
            per_worker_id_job_assignment[worker_id].append(
                (job_id, worker_type_order)
            )
        ...
        for worker_id in range(n):
            if len(per_worker_id_job_assignment[worker_id]) > 0:
                ...
                allocation[job_id][worker_type] = 1.0 / scale_factors[job_id]
