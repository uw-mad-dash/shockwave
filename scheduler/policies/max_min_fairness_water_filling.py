...
class WaterFillingAlgorithm:
    ...
    def _compute_priority_weights(
        ...
        if entity_to_job_mapping is None:
            raise ValueError(
                "entity_to_job_mapping cannot be None when "
            ...
            if priority_reweighting_policy == "fairness":
                ...
                for job_id in entity_to_job_mapping[entity_id]:
                    if job_id in final_normalized_effective_throughputs:
                        continue
                    total_job_priority_in_entity += float(
    ...
    def _get_allocation(
        ...
        normalized_effective_throughputs_so_far_parameter = self._lp_variables_and_parameters[
            "normalized_effective_throughputs_so_far_parameter"
        ]
        normalized_effective_throughputs_lower_bounds_parameter = self._lp_variables_and_parameters[
            "normalized_effective_throughputs_lower_bounds_parameter"
        ]
        multiplicative_terms_parameter = self._lp_variables_and_parameters[
            "multiplicative_terms_parameter"
        ]
        additive_terms_parameter = self._lp_variables_and_parameters[
            "additive_terms_parameter"
        ]
        ...
        for i, job_id in enumerate(job_ids):
            if job_id not in final_normalized_effective_throughputs:
                if priority_weights[i] > 0.0:
                    multiplicative_terms[i] = (
                        priority_weights[i] * scale_factors_array[i, 0]
                    )
        ...
        if self._lp is None:
            self._lp_objective = cp.Maximize(
                cp.min(
                    cp.multiply(
                        objective_terms, multiplicative_terms_parameter
                    )
                    + additive_terms_parameter
                )
            )
    ...
    def _get_bottleneck_jobs(
        ...
        if self._milp is None:
            ...
            self._milp_variables_and_parameters["z"] = cp.Variable(
                len(job_ids), boolean=True
            )
            self._milp_variables_and_parameters[
                "normalized_effective_throughputs_so_far_parameter"
            ] = cp.Parameter(len(job_ids))
        ...
        normalized_effective_throughputs_so_far_parameter = self._milp_variables_and_parameters[
            "normalized_effective_throughputs_so_far_parameter"
        ]
        normalized_effective_throughputs_lower_bounds_parameter = self._milp_variables_and_parameters[
            "normalized_effective_throughputs_lower_bounds_parameter"
        ]
        ...
        if self._milp.status != "optimal":
            raise Exception(
                "WARNING: Non-optimal allocation in _get_bottleneck_jobs()!"
            )

        return z.value

    def _run_get_allocation_iterations(
        self,
        job_ids,
        m,
        n,
        ...
        while not done:
            ...
            if verbose:
                print(
                    "Using the following as priority weights:",
                    np.array([priority_weights[job_id] for job_id in job_ids]),
                )
            priority_weights = np.array(
                [
                    1.0 / priority_weights[job_id]
                    if priority_weights[job_id] > 0
                    else 0.0
            ...
            try:
                ...
                if x is None:
                    ...
                    normalized_effective_throughputs_so_far += np.multiply(
                        mask, c
                    )
            except:
                x, c, mask = old_x, old_c, old_mask
            ...
            old_len_effective_throughputs = len(
                final_normalized_effective_throughputs
            )
            for i, job_id in enumerate(job_ids):
                if (
                    job_id not in final_normalized_effective_throughputs
                    and (z is None or not z[i])
            ...
            if old_len_effective_throughputs == len(
                final_normalized_effective_throughputs
            ):
                done = True
            num_iterations += 1
...
class MaxMinFairnessWaterFillingPolicy(Policy, WaterFillingAlgorithm):
    def __init__(self, priority_reweighting_policies=None):
        ...
        self._max_min_fairness_perf_policy = MaxMinFairnessWaterFillingPolicyWithPerf(
    ...
    def get_allocation(
        ...
        unflattened_x = self._max_min_fairness_perf_policy.get_allocation(
            new_unflattened_throughputs,
            scale_factors,
            unflattened_priority_weights,
            cluster_spec,
        ...
        if return_effective_throughputs:
            ...
            proportional_throughputs = self._max_min_fairness_perf_policy._proportional_policy.get_throughputs(
...
class MaxMinFairnessWaterFillingPolicyWithPerf(Policy, WaterFillingAlgorithm):
    ...
    def get_allocation(
        ...
        self._get_effective_throughputs = lambda x: cp.sum(
            cp.multiply(throughputs, x), axis=1
        )
        ...
        if verbose:
            print(
                "Normalized effective throughputs:",
                normalized_effective_throughputs,
            )
            print(
                "Constraints:",
...
class MaxMinFairnessWaterFillingPolicyWithPacking(
    ...
    def get_allocation(
        ...
        all_throughputs, index = self.flatten(
            d=unflattened_throughputs, cluster_spec=cluster_spec
        )
        ...
        for i, single_job_id in enumerate(single_job_ids):
            for j, worker_type in enumerate(worker_types):
                throughputs_no_packed_jobs[i, j] = unflattened_throughputs[
                    single_job_id
                ][worker_type]
        proportional_throughputs = self._proportional_policy.get_throughputs(
        ...
        self._get_effective_throughputs = lambda x: self._get_effective_throughputs_helper(
            x, all_throughputs, index
        )
        ...
        priority_weights = np.array(
            [
                1.0 / unflattened_priority_weights[single_job_id]
