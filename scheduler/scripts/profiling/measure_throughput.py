...
class Profiler:
        ...
        while True:
            ...
            if self._log_file is not None:
                with open(self._log_file, "a") as f:
                    f.write("[%s] %s" % (str(datetime.datetime.now()), output))
    ...
    def _initialize_completed_steps_and_elapsed_time(
        self, job_id=None, worker_type=None
    ):
    ...
    def _initialize_per_worker_type_task_queues(
        self, isolated, packing, distributed
    ):
            ...
            if isolated:
                for worker_type in per_worker_type_task_queues:
                    ...
                    for i in range(len(JobTable)):
                        ...
                        if (
                            scale_factor > 1
                            and not self._can_be_run_multi_gpu(job_type)
                        ):
                            continue
        ...
        with self._lock:
            ...
            for i in range(num_gpus):
                ...
                self._write_queue.put(
                    "Registered worker %d (%s) at %s:%s"
                    % (worker_id, worker_type, ip_addr, port)
                )
    ...
    def _update_lease_callback(
        ...
        if steps == 0 or duration == 0:
            ...
            with self._lock:
                ...
                if update_id == 0:
                    if (
                        job_id in self._max_steps
                        and worker_type in self._max_steps[job_id]
                    ):
            ...
            if update_id == 0:
                with self._lock:
                    ...
                    max_completed_steps = max(
                        [
                            request[0]
                            for request in self._lease_update_requests[job_id]
                        ]
                    )
    ...
    def _done_callback(
        ...
        with self._lock:
            ...
            job_throughputs = []
            for (num_steps, execution_time) in zip(
                all_num_steps, all_execution_times
            ):
            ...
            if job_failed:
                ...
                if job_id.is_pair():
                    for i in range(len(job_throughputs)):
                        ...
                        all_throughputs[job_types[1]][job_types[0]][
                            1 - i
                        ] += throughput
                else:
                    all_throughputs[job_types[0]][
                        job_types[1]
                    ] += job_throughputs[0]
            ...
            if self._num_throughput_updates[job_id] == scale_factor:
                updated_throughputs = all_throughputs[job_types[0]][
                    job_types[1]
                ]
                throughputs_str = str(updated_throughputs)
                if job_id.is_pair():
                    self._write_queue.put(
                        "Throughputs for %s on %s: %s"
                        % (str(job_types), worker_type, throughputs_str)
                    )
                else:
                    assert job_types[1] is None
                    self._write_queue.put(
                        "Throughput for %s on %s: %s"
                        % (job_types[0], worker_type, throughputs_str)
                    )
    ...
    def profile(self, isolated, packed, distributed):
        ...
        per_worker_type_task_queues = self._initialize_per_worker_type_task_queues(
            isolated, packed, distributed
        )
        ...
        while not done:
            ...
            for worker_type in self._cluster_spec:
                ...
                while (
                    num_remaining_worker_ids > 0
                    and not per_worker_type_task_queues[worker_type].empty()
                ):
                    (task, scale_factor) = per_worker_type_task_queues[
                        worker_type
                    ].get()
                    ...
                    # Schedule the task.
                    for i in range(
                        worker_id_ptr, worker_id_ptr + scale_factor
                    ):
                        ...
                        for j, job_description in enumerate(task):
                            ...
                            self._write_queue.put(
                                ...
                                "worker %d (%s)"
                                % (job_id, job_type, worker_id, worker_type)
                            )
            ...
            for job_type in self._throughputs[worker_type]:
                ...
                for other_job_type in self._throughputs[worker_type][job_type]:
                    if other_job_type is None:
                        ...
                        other_key = str(
                            utils.parse_job_type_str(other_job_type)
                        )
                    throughputs[worker_type][key][
                        other_key
                    ] = self._throughputs[worker_type][job_type][
...
def main(args):
    if not args.isolated and not args.packed:
        raise ValueError(
            'At least one of "--isolated" or "--packed"' "must be set"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--log_file", type=str, default=None, help="Log file"
    )
    parser.add_argument(
        "-i",
        "--isolated",
