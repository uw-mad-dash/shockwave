...
def main(args):
    if (
        args.num_total_jobs_lower_bound is None
        and args.num_total_jobs_upper_bound is not None
    ) or (
        args.num_total_jobs_lower_bound is not None
        and args.num_total_jobs_upper_bound is None
    ):
        raise ValueError(
            "If num_total_jobs range is not None, both "
    ...
    for cluster_spec_str in args.cluster_spec:
        ...
        for policy_name in policy_names:
            ...
            all_num_total_jobs = np.linspace(
                lower_bound, upper_bound, args.num_data_points
            )
            ...
            for num_total_jobs in all_num_total_jobs:
                ...
                for seed in args.seeds:
                    ...
                    raw_logs_seed_subdir = os.path.join(
                        raw_logs_policy_subdir, seed_str
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("If set, schedules jobs regardless of " "worker type"),
    )
    parser.add_argument(
        "--ideal",
        action="store_true",
