...
def main(args):
    ...
    if (
        args.throughput_lower_bound is None
        or args.throughput_upper_bound is None
    ):
    ...
    estimate_throughputs = min(profiling_percentages) < 1.0 or min(
        all_num_reference_models
    ) < len(JobTable)
    ...
    for cluster_spec_str in args.cluster_spec:
        ...
        raw_logs_cluster_spec_subdir = os.path.join(
            raw_logs_dir,
            "v100=%d.p100=%d.k80=%d"
            % (
                cluster_spec["v100"],
        ...
        for policy_name in policy_names:
            raw_logs_policy_subdir = os.path.join(
                raw_logs_cluster_spec_subdir, policy_name
            )
            ...
            for profiling_percentage in profiling_percentages:
                if estimate_throughputs:
                    profiling_percentage_str = "profiling_percentage=%f" % (
                        profiling_percentage
                    )
                    raw_logs_profiling_subdir = os.path.join(
                        raw_logs_policy_subdir, profiling_percentage_str
                    )
                ...
                for i, num_reference_models in enumerate(
                    args.num_reference_models
                ):
                    if estimate_throughputs:
                        num_reference_models_str = (
                            "num_reference_models=%d" % (num_reference_models)
                        )
                        raw_logs_num_reference_models_subdir = os.path.join(
                            raw_logs_profiling_subdir, num_reference_models_str
                        )
                    ...
                    for throughput in throughputs:
                        if (
                            cluster_spec_str in cutoff_throughputs
                            and policy_name
                            in cutoff_throughputs[cluster_spec_str]
                        ):
                            cutoff_throughput = cutoff_throughputs[
                                cluster_spec_str
                            ][policy_name]
                            if throughput >= cutoff_throughput:
                                print(
                                    "Throughput of %f is too high "
                        ...
                        for seed in args.seeds:
                            ...
                            raw_logs_seed_subdir = os.path.join(
                                raw_logs_num_reference_models_subdir, seed_str
                            )
    ...
    if len(all_args_list) > 0:
        current_time = datetime.datetime.now()
        print(
            "[%s] Running %d total experiment(s)..."
            % (current_time, len(all_args_list))
        )
        with multiprocessing.Pool(args.processes) as p:
            ...
            results = [
                p.apply_async(simulate_with_timeout, args_list)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sweep through lambda values")
    fixed_range = parser.add_argument_group("Sweep over fixed range")

    parser.add_argument(
        "-l", "--log-dir", type=str, default="logs", help="Log directory"
    )
    parser.add_argument(
        "-s",
        "--window-start",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If set, fixes the duration of all jobs to the "
            "specified value (in seconds)"
        ),
    )
    parser.add_argument(
        "--cutoff-throughputs-file",
        type=str,
        default=None,
        help=(
            "If set, uses the attached cutoff_throughputs "
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "If set, adds as many jobs as there are workers "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Checkpoint threshold, None if checkpointing is "
            "disabled. Checkpoint is created after this "
            "job ID is added."
        ),
    )
    parser.add_argument(
        "--profiling_percentages",
        type=float,
        nargs="+",
        default=[1.0],
        help=(
            "Percentages of machines dedicated to profiling "
