...
class Job:
    ...
    def __str__(self):
        ...
        for i, (worker_type, start_time, end_time, allocation) in enumerate(
            metadata
        ):
            s += "\t%2d) [%4s] %.3f - %.3f (%.3f) (%s)\n" % (
                i,
...
def get_overall_execution_time(jobs, max_end_time=None):
    ...
    for job_id in jobs:
        if (
            earliest_dispatch_time is None
            or jobs[job_id].dispatch_time < earliest_dispatch_time
        ):
            earliest_dispatch_time = jobs[job_id].dispatch_time
        if (
            latest_end_time is None
            or jobs[job_id].end_times[-1] > latest_end_time
        ):
...
def get_worker_runtimes(jobs, num_jobs=None):
    ...
    for job_id in jobs:
        job = jobs[job_id]
        metadata = zip(
            job.start_times, job.end_times, job.worker_ids, job.worker_types
        )
...
def print_job_summary(jobs, num_jobs):
    ...
    worker_runtimes, restricted_execution_time = get_worker_runtimes(
        jobs, num_jobs
    )
    print(
        "Average job completion time: %.3f" % (np.mean(job_completion_times))
    )
    print("Average queuing delay: %.3f" % (np.mean(queueing_delays)))
    print(
        ("Queueing delay / (Queueing delay + Computation time): " "%.3f")
        % (
            sum(queueing_delays)
    ...
    for logfile in jobs:
        ...
        for job_id in jobs[logfile]:
            job_completion_times.append(
                jobs[logfile][job_id].completion_time()
            )
        num_bins = 20
        counts, bin_edges = np.histogram(
            job_completion_times, bins=num_bins, normed=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The maximum number of completed jobs to "
