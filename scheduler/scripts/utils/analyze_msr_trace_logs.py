...
def get_logfile_paths(directory_name):
    ...
    for logfile_path in get_logfile_paths_helper(directory_name):
        ...
        policy = m.group(5)
        seed = int(m.group(6))
        logfile_paths.append(
            (vc, v100s, p100s, k80s, policy, seed, logfile_path)
        )
    return logfile_paths
...
def average_jct_fn(logfile_path, min_job_id=None, max_job_id=None):
    ...
    with open(logfile_path, "r") as f:
        ...
        for line in lines[-10000:]:
            m = re.match(
                r"Average job completion time: (\d+\.\d+) seconds", line
            )
            if m is not None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze Philly traces")

    parser.add_argument(
        "-l", "--logdir-path", type=str, default="logs", help="Log directory"
    )
