...
BASE_SCHEDULER_COMMAND = """python scripts/drivers/run_scheduler_with_trace.py \
        --seed 0 \
        --solver ECOS \
...
def main(args):
    ...
    for (model, model_dir) in models:
        ...
        for num_jobs in [1, 2]:
            job = Job(
                job_id=JobIdPair(num_jobs - 1, None),
                job_type=model,
            ...
            num_jobs_dir = os.path.join(
                model_dir, "num_jobs={0}".format(num_jobs)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_dir", type=str, required=True, help="Directory to run jobs from"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
