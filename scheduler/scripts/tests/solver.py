...
def get_job_throughputs(jobs, oracle_throughputs, worker_types):
    ...
    for i, job in enumerate(jobs):
        ...
        for j, other_job in enumerate(jobs[i + 1 :]):
            ...
            for worker_type in worker_types:
                throughputs[merged_job_id][worker_type] = oracle_throughputs[
                    worker_type
                ][job_type_key][other_job_type_key]
    return throughputs
    ...
    for job_type_key in job_type_keys:
        ...
        for worker_type in worker_types:
            ...
            for other_job_type_key in job_type_keys:
                ...
                job_type_throughputs[job_type_key][worker_type][
                    other_job_type_key
                ] = oracle_throughputs[worker_type][job_type_key][
                    other_job_type_key
                ][
                    0
                ]
...
def get_allocation(
    ...
    unflattened_allocation = policy.get_allocation(
...
def get_allocation_using_job_type_throughputs(
    ...
    job_type_throughputs = get_job_type_throughputs(
        jobs, oracle_throughputs, worker_types
    )
    unflattened_allocation = policy.get_allocation_using_job_type_throughputs(
        job_type_throughputs,
        job_id_to_job_type_key,
        scale_factors,
        priority_weights,
...
def print_effective_throughputs(
    allocation, throughputs, job_id_to_job_type_key
):
    ...
    for single_job_id in single_job_ids:
        ...
        for worker_type in worker_types:
            normalizing_factors[single_job_id] += throughputs[worker_type][
                job_type_key
            ]["null"]
        ...
        for job_id in relevant_job_ids[single_job_id]:
            for worker_type in worker_types:
                if not job_id.is_pair():
                    ...
                    other_job_id = job_id.singletons()[1 - index]
                    other_job_type_key = job_id_to_job_type_key[other_job_id]
                    if other_job_type_key[1] != job_type_key[1]:
                        continue
                    throughput = throughputs[worker_type][job_type_key][
                        other_job_type_key
                    ][0]
...
def main(args):
    ...
    original_allocation = get_allocation(
        policy,
        jobs,
        oracle_throughputs,
        cluster_spec,
    ...
    start = datetime.datetime.now()
    job_type_allocation = get_allocation_using_job_type_throughputs(
        policy,
        jobs,
        oracle_throughputs,
        cluster_spec,
    ...
    print_effective_throughputs(
        original_allocation, oracle_throughputs, job_id_to_job_type_key
    )
    print("")

    print("Effective_throughputs using job type throughputs:")
    print_effective_throughputs(
        job_type_allocation, oracle_throughputs, job_id_to_job_type_key
    )
    ...
    print(
        "Runtime using job type throughputs:",
