...
class Worker:
    def __init__(
        ...
        if num_gpus > num_available_gpus:
            raise ValueError(
                "%d GPUs requested active, but only %d total "
                "GPUs are available" % (num_gpus, num_available_gpus)
            )
        ...
        self._server_thread = threading.Thread(
            target=worker_server.serve, args=(worker_port, callbacks,)
        )
    ...
    def _run_job_callback(self, jobs, worker_id, round_id):
        ...
        self._logger.debug("_run_job_callback is invoked")
        while True:
            try:
                self._dispatcher
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--worker_type", type=str, required=True, help="Worker type"
    )
    parser.add_argument(
        "-i",
        "--ip_addr",
