...
parser.add_argument('--checkpoint_dir', default='/home/nfs/gavel/checkpoints/resnet-18',
...
if args.master_addr is not None:
    ...
    #os.environ['NCCL_LAUNCH_MODE']='cooperative groups'
    #os.environ['NCCL_LAUNCH_MODE']='GROUP'
    # option 1: use InfiniBand for distributed jobs
    #socket_ifname = 'enps0f0' if socket.gethostname() != 'node1' else 'enp94s0f0'
    #os.environ['NCCL_SOCKET_IFNAME'] = socket_ifname
    # option 2: use sockets for distributed jobs
    ...
    print(f"Initializing process group with world_size {args.world_size} from rank {args.rank}")
    dist.init_process_group(backend=args.dist_backend,
                            init_method=args.dist_url,
                            world_size=args.world_size,
                            rank=args.rank)
    print(f"Initialized process group, building DDP model with device {args.local_rank}")
...
def load_checkpoint(args, checkpoint_path):
    ...
    if os.path.exists(checkpoint_path):
        try:
            print("checkpoint path for args.local_rank", args.local_rank)
            checkpoint = torch.load(checkpoint_path,
                                    map_location='cuda:{}'.format(args.local_rank))
            print('==> Resuming from checkpoint at %s...' % (checkpoint_path))
            print("after loading checkpoint", args.local_rank)
            return checkpoint
...
def save_checkpoint(checkpoint_path, state):
    ...
    print("args.local rank in save checkpoint is",args.local_rank) 
    torch.save(state, checkpoint_path)
    print("after save checkpoint", args.local_rank)
...
if args.enable_gavel_iterator:
    ...
    print("checkpoint is", args.local_rank)
else:
...
for epoch in range(start_epoch, args.num_epochs):
    ...
    if args.enable_gavel_iterator:
        if trainloader.done:
            print("train loader done", args.local_rank)
            break
        elif done:
            trainloader.complete()
            print("train locader complete", args.local_rank)
            break
    elif done:
        break
...
if not args.distributed or args.rank == 0:
    if args.enable_gavel_iterator:
        ...
        print("save checkpoint else", args.local_rank)
