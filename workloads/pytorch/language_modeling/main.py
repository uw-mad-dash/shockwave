...
parser.add_argument('--checkpoint_dir', type=str,
                    default='/home/nfs/gavel/checkpoints',
                    help='Checkpoint dir')
...
def load_checkpoint(args, checkpoint_path):
    try:
        ...
        print("load checkpiint", args.local_rank)
        with open(checkpoint_path, 'rb') as f:
            state = torch.load(f, map_location='cuda:{}'.format(args.local_rank))
            print("after ch", args.local_rank)
            return state
...
def save_checkpoint(state, checkpoint_path):
    with open(checkpoint_path, 'wb') as f:
        ...
        print("before save ch", args.local_rank)
        torch.save(state, f)
        print("after save", args.local_rank)
...
if args.enable_gavel_iterator:
    ...
    print("train loader args", args.local_rank)

state = None
if args.checkpoint_dir is not None:
    if not os.path.isdir(args.checkpoint_dir):
        os.mkdir(args.checkpoint_dir)
        print("checkpoint args local rank is", args.local_rank)
    else:
        ...
        if os.path.exists(checkpoint_path):
            if args.enable_gavel_iterator:
                ...
                print("state is", args.local_rank)
            else:
                state = load_checkpoint(args, checkpoint_path)
                print("state else is", args.local_rank)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.enable_gavel_iterator:
            if train_loader.done:
                ...
                print("complete", args.local_rank)
                break
        elif done:
          break
        print('-' * 89)
